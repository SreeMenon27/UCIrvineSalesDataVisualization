...
class ReportGenerator:
    ...
    def _build_level_1_content(self, data):
        ...
        col_table = Table(col_data, colWidths=[120, 400], hAlign='LEFT')
        col_table.setStyle([
    ...
    def generate_level_2_report(self, data_level1, data_level2, output_file):
        ...
        for key, title in plot_titles.items():
            if key in data_level2:
                block = KeepTogether([
                    Paragraph(f"🖼️ {title}", styles["Heading2"]),
                    Spacer(1, 6),
                    Image(data_level2[key], width=6.0*inch, height=3.0*inch),
                    Spacer(1, 12)
                ])
                story.append(block)
    ...
    def generate_level_3_report(self, data_level1, data_level2, data_level3, output_file):
        ...
        plot_titles_level2 = {
            ...
            "monthly_revenue_plot": "Monthly Revenue Trend",
            "product_quantity_plot": "Top 10 Products by Quantity Sold",
            "top_customers_plot": "Top 10 Customers by Revenue"  # ✅ Added this line
        }
        ...
        for key, title in plot_titles_level3.items():
            if key in data_level3:
                block = KeepTogether([
                    Paragraph(f"🖼️ {title}", styles["Heading2"]),
                    Spacer(1, 6),
                    Image(data_level3[key], width=6.0 * inch, height=3.0 * inch),
                    Spacer(1, 12)
                ])
                story.append(block)
